...
class Edge(object):
  ...
  def make_samples(self, db, verbose=True):
    if h_type == 'global':
      sample_cache = "edge-{}-stride".format(h_type)
    elif h_type == 'region':
      sample_cache = "edge-{}-stride-n_slice".format(h_type)
    ...
    try:
      ...
      for d in data.itertuples():
        ...
        exists = os.path.isfile(d_img)
        if exists:
          print("File '%s' of class '%s' exists. Appending to samples..." % (d_img, d_cls))
          ...
          print("File '%s' of class '%s' does no exist. Skipping." % (d_img, d_cls))
